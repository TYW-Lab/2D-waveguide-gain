...
class WaveguideSimulation():
    def __init__(self, neff, Lx, Lz, Nx, Nz, diffusion_length, Ncom=1, I_sat=float('inf')):
    ...
    def run_simulation(self, a, delta_n = None):
        ...
        for each z:    
            ...
            delta_n_static_slice = ?? # static delta_n due to waveguide structure
            delta_n_active_slice = material.update(current_slice, P_slice) # delta_n due to current injection
            delta_n_slice = delta_n_static_slice + delta_n_active_slice
        
        
        
        for z_ind, (delta_n_slice, current_slice) in enumerate(zip(delta_n_static, current)):
            delta_n_active_slice = self.update_delta_n_slice(current_slice, a)
            ...
    ...
    def _plot_delta_n(self, xlim=200, steady_state = False, real = True):
        wg = self
        if steady_state: delta_n = wg.delta_n_SS
